...
-- Query to validate the assigned keys
SELECT 
...
--category-dish MANY-TO-MANY relationship by the cross-referencing 
...
-- copy INSERT from projectdata.sql
/* 
 --------------------------------------------
...
--Query to display the restaurant name, street number, street name and telephone number
SELECT name, telephone, street_name, street_number FROM restaurant, address;

--Query to display the best ratings
SELECT MAX(rating) AS best_rating FROM review;

--Query to display category, dish_name and price sorted by dish name
...
--Query to display category, dish_name and price sorted by category name
SELECT category.name AS category, dish.name AS dish_name, categories_dishes.price FROM category, dish, categories_dishes
WHERE categories_dishes.dish_id = dish.id AND categories_dishes.category_id = category.id
ORDER BY category.name;

--Query to display all spicy dishes, their prices and category
...
--Query to display the dish_id and the count dish_id from the categories_dishes table
SELECT dish_id, COUNT(dish_id) AS dish_count FROM categories_dishes
GROUP BY 1 ORDER BY 1;

--Query to display only the dishes form the categories_dishes table
SELECT dish_id, COUNT(dish_id) AS dish_count FROM categories_dishes
GROUP BY 1 HAVING COUNT(dish_id) > 1 ORDER BY 1;

--Query to display multiple names and dish counts
...
--Query to display the best ratings and its description
